...
const lintSetup = (scriptBody) =>
	linter.verify(
		`<script setup lang="ts">\n${scriptBody}\n</script>\n<template><div /></template>\n`,
		{
			...
			rules: { "no-restricted-syntax": ["error", ...dataLoadingFetchRestrictions] },
		},
	);

const flagged = [
	["top-level await $fetch (const)", `const data = await $fetch("/api/thing");`],
	["top-level await $fetch (statement)", `await $fetch("/api/thing");`],
	[
		"$fetch in onMounted",
		`onMounted(async () => {\n\tconst x = await $fetch("/api/thing");\n\treturn x;\n});`,
	],
	["$fetch in onServerPrefetch", `onServerPrefetch(async () => {\n\tawait $fetch("/api/thing");\n});`],
];

const allowed = [
	["useFetch", `const { data } = await useFetch("/api/thing");`],
	["useAsyncData wrapping $fetch", `const { data } = await useAsyncData("k", () => $fetch("/api/x"));`],
	[
